...
with open("/storage/.kodi/userdata/autoexec.py","a") as log:
    log.write("os.system(\"kodi-send --action='RunScript(\\\"/storage/ConnectingProgram/de/set-settings_de.py\\\")'\")\n")

setRemote = xbmcgui.Dialog().yesno("Konfiguration","Möchten Sie eine Fernbedienung einrichten?")
if setRemote == True:
    os.system("kodi-send --action='RunScript(\"/storage/ConnectingProgram/de/remote-control_de.py\")'")
if setRemote == False:
