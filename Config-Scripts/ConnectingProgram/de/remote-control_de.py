...
while not monitor.abortRequested():
	...
	win.addControl ( xbmcgui.ControlLabel ( x = 190 ,  y = y , width = 750 ,  height = 25 ,  label = 'Drücken Sie die Tasten Ihrer Fernebdienung mehrere Male.'))
	win.show()
	y += 20

	process=subprocess.Popen("ir-keytable -t >> /storage/IRlog.txt", shell=True)
	pDialog = xbmcgui.DialogProgressBG()
	pDialog.create('Taste hinzufügen', 'Drücken Sie einen Taste Ihrer Fernebdienung mehrere Male')
	...
	if Cancel == False:
		...
		os.system("kodi-send --action='RunScript(\"/storage/ConnectingProgram/de/end_de.py\")'")
	else:
		conffile = open("/storage/.config/rc_maps.cfg", "w")
		conffile.write("")
		conffile.close()
